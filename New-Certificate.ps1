...
Function New-WildcardCertificate {
    ...
    Begin {
        ...
        (Get-Content $PSScriptRoot\ssl.ini) | Foreach-Object {$_ -replace 'ServerFQDN',"$CertificateName"}  | Out-File .\Wildcard.ini
    }
    Process {
        If ($CAName -eq "") {
...
New-WildcardCertificate -Path C:\Install -PFXPath "\\MDT01\d$\Shares" -Password "Password1" -CertificateName "Server1.citrixlab.dk" -Verbose 